...
class LabelTransferPredictor (LabelTransferTrainer):
    ...
    def _load_all_data(self, dataset: str) -> pd.DataFrame:
        ...
        expr_data = pd.read_csv(f"{self.data_dir}{dataset}_expression.csv", index_col=0)
        expr_data = expr_data.T        
        expr_data.reset_index(drop=True, inplace=True)
        labels_data = pd.read_csv(f"{self.data_dir}{dataset}_clusters.csv", index_col=None)        
    ...
    def predict_dataset(self, dataset: str, threshold: float = 0.7) -> pd.DataFrame:
        ...
        for model_dataset in [k for k in self.models.keys() if k[-1] == sex]:           
            predictions = self.models[model_dataset].predict(expr_data.drop(columns=['Cluster', 'Barcode']))
    ...
    def plot_predictions_on_UMAP(self, dataset: str, predictions: pd.DataFrame, save: bool = False) -> None:
        """
